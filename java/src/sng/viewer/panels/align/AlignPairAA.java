...
***********************************************************************/
/**************************************************
 * my changes are remarked with CASX 
 * CASX -- PairAlign class interfaces to TCW 
   1 (new SWSmart (sub, 8,     seq1, seq2)).domatch(out, "SMART LOCAL")
   2 (new SWSmartAffine (sub, 12, 2, seq1, seq2)).domatch(out, "SMART AFFINE LOCAL");
 */
public class AlignPairAA {
	private String alignOne, alignTwo, alignMatch;
	private int [] ends;
	private int score;
	...
	public AlignPairAA(int gap, int extend, String seq1, String seq2, int type) {
		 ...
		 if (type==1) {
			 method = "Local";
			 saf = new SWSmart (sub, gap, seq1, seq2);
		 }
		 else {
			 method = "Local with affine gaps";
			 saf = new SWSmartAffine (sub, gap, extend, seq1, seq2);
		 }
		 
		 String[] match = saf.getMatch();
		 alignOne = match[0];
		 alignTwo = match[1];
	...
	public String getAlignOne() {return alignOne;}
	public String getAlignTwo() {return alignTwo;}
	public String getAlignMatch() {return alignMatch;}
...
// CASX added last row for '*' for stop 
...
abstract class Align {
	  ...
	  public String strip(String s) {
	    ...
	    for (int i=0; i<residues.length(); i++) {
	      ...
	      if (c < 96) valid[c] = valid[c+32] = true;
	      else valid[c-32] = valid[c] = true;
	    }
	  ...
	  public String[] getMatch() {
	    ...
	    while ((tb = next(tb)) != null) {
	      ...
	      else 				res1.append(seq1.charAt(i-1)); 
	      if (j == tb.j) 	res2.append('-');
	      else				res2.append(seq2.charAt(j-1)); 
	      i = tb.i; j = tb.j;
	    }        
	  ...
	  // CASX returns array of ends
	  public abstract int [] getEnds();

	  // Print the matrix (matrices) used to compute the alignment
	  public abstract void printf(Output out);

// CASX next four methods were static. Eclipse didn't like
	  // Auxiliary functions 
...
abstract class AlignSimple extends Align {
	  ...
	  public int [] getEnds() { return null;} // CASX - TCW not using, so not working
}
...
abstract class AlignAffine extends Align {
	  ...
	  // CASX not using -- so not tested
	  public int [] getEnds() {
		  int [] x = {0, seq1.length(), 0, seq2.length()};
		  return x;
	  }
}
...
class NWSmart extends AlignSmart {
	  ...
	  // CASX not using so not tested
	  public int [] getEnds() {
		  int [] x = {0, seq2.length(), 0, seq1.length()};
		  return x;
	  }
	  
} // end NWsmart

// XXX Local alignment with the Smith-Waterman algorithm (simple gap
	  ...
	  public SWSmartAffine(Substitution sub, int d, int e, 
	    ...
	    for (int i=1; i<=n; i++) {
	      ...
	      for (int j=1; j<=m; j++) {
	        ...
	        if (val == 0)			start[1][j] = new Traceback2(i, j);
	        else if (val == valm)	start[1][j] = start[0][j-1];
	        else if (val == valix)	start[1][j] = start[0][j];
	        else if (val == valiy)	start[1][j] = start[1][j-1];
	        else 
