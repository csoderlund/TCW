...
public class AlignBasePanel extends JPanel
{
	public AlignBasePanel ( Font baseFont )
	{
		theFont = baseFont;
											...
											double dYBottom  ) 
	{
		...
		{		
			...
			if (bCtgDisplay) {
				if ( getIsNAt(i) ) 						baseColor = Globalx.purple; // just in consensus
				else if ( getIsMismatchAt( sequence, i ) )		baseColor = Globalx.mismatchRed;
				else if ( getIsGapAt( sequence, i )) 			baseColor = Globalx.darkGreen;
				else if ( getIsLowQualityAt ( sequence, i ) )	baseColor = Globalx.lowQualityBlue;

			} else {
				if ( getIsStop ( sequence, i ) )					baseColor = Color.gray;
				else if ( getIsNAt ( sequence, i ) )				baseColor = Globalx.mediumGray;  
										...
										double dYBottom)
	{
		...
		while(sequence.charAt(startGapEnd) == Globalx.gapCh && startGapEnd<sequence.length()) startGapEnd++;
		if (startGapEnd==sequence.length()) {
			System.out.println("All gaps '" + sequence + "'");
			startGapEnd=0;
		}
		else {
			while(sequence.charAt(endGapStart-1) == Globalx.gapCh && endGapStart>0) endGapStart--;
		}
	...
	protected boolean getIsHighSubAt ( String seq, int nPos ) { return false; }
